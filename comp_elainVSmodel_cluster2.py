...
fo.write(f'6, 1.5, 26.333, {round(3.3,2)}, {round(3.1473177700386983,2)}, {round(np.abs((3.1473177700386983-3.3)/(3.3))*100,2)}\n')
fo.write(f'4, 1.333, 30.0, {round(4.0,2)}, {round(3.8971280594764726,2)}, {round(np.abs((3.8971280594764726-4.0)/(4.0))*100,2)}\n')
fo.write(f'3, 1.54, 11.5,  {round(80/7,2)},{round(9.631248145576679,2)}, {round(np.abs((9.631248145576679-80/7)/(80/7))*100,2)}\n')
fo.close()
...
plt.gca().add_artist(plt.legend([j1,j2],['Our model','simulation'],title='Lu et al. :$\gamma$=1.333; Q=26.333',bbox_to_anchor=(1.4,1.0)))#,fontsize=18)) ,loc='upper left'))#
################
...
plt.gca().add_artist(plt.legend([l1,l2],['Our model','simulation'],title='  high $\gamma$ : $\gamma$ =1.5 ; Q=26.333   ',bbox_to_anchor=(1.4,0.75)))#,fontsize=18)),loc=0
...
plt.gca().add_artist(plt.legend([t1,t2],['Our model','simulation'],title='    $Q_{up}$  :$\gamma$=1.333 ;  Q=30.0     ',bbox_to_anchor=(1.4,0.5)))#,fontsize=18)),loc='upper center'))#
################
...
plt.gca().add_artist(plt.legend([k1,k2],['Our model','simulation'],title='Sharpe et al. :$\gamma$=1.54;Q=11.5',bbox_to_anchor=(1.4,0.25)))#,fontsize=18)),loc='lower right'))#
#####################
