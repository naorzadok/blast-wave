...
def find_od(ood):
    ...
    T4        = (P4/P0)**((gamma-1)/gamma)
    a_rat     = np.sqrt(T0/T4)
    M0        = (D0*ood)/C0
    P2        = ((1+(2*gamma)/(gamma+1)*(M0**2-1)))*P0
    print(ood,P4,T4,M0)
    return (P4/P0)-(P2/P0)*(1-(((gamma-1)*a_rat*(P2/P0-1))/(np.sqrt(2*gamma*(2*gamma+(gamma+1)*(P2/P0-1))))))**(-(2*gamma)/(gamma-1))

def blast(AA,eff,T00,gam,qq,moll,P00):
    ############################################
        #units SI
    global D0, C0, gamma,rho0,T0,P0,RR,Rr,alpha,h,A,Ea,q,rho_vn0, T_vn0, P_vn0, r_l, M_l, L, P3, od, r0, h, alpha, P0
    vec       = [1500,2500,2000]
    ...
    od        = opt.root_scalar(find_od,method='secant',x0=1.6,x1=1.7)
    print(od)
    od = od.root
