...
def find_t_star(t_star):
    ...
    return ((t_star+tau)-opt.fsolve(t_f,1.2*t_star))
...
def geo_mod(d0): #calculate r at t[0,t*]
    ...
    r_star    = r(t_star)#(r0)*np.sqrt((1+od**2))
    t_end      = opt.fsolve(t_final,1.2*t_star)
    #print(t_star,'1')
    if t_end<1e-12:
        return 1000
    time_jump  = t_star/1000
...
def blast(AA,eff,T00,gam,qq,moll,P00,odd):
    ...
    d0 =  opt.root_scalar(geo_mod,method='secant',x0=0.8,x1=0.25)
    #d0  =opt.bisect(geo_mod,0.0005,0.5)#,xtol=0.00001)
    #d0 = 0.005#0.005
    #r0        = d0.x/2
    r0        = d0.root/2
    ############################################
    t_star    = opt.fsolve(find_t_star,20*r0/D0)#t_star    = ((np.sqrt(2)-1))*(od*r0/D0)opt.bisect(find_t_star,0.1*r0/D0,10*r0/D0)#
    r_star    = r(t_star)#(r0)*np.sqrt((1+od**2))
    t_end      = opt.fsolve(t_final,1.2*t_star)
    #print(t_end,t_star)
    time_jump  = t_star/1000
    ...
    '''
    return[L,W,math.degrees(theta.x),od,r0]
'''
