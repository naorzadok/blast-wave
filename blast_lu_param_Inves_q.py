...
q_l = np.linspace(15,50,30)
print('q loop')
d0_l, L_l, W_l, theta_l = [],[],[],[]
#gamma loop
print(dt.datetime.now())
for loop in q_l:
    x = blast(lu_et.get(f'eff{c[j]}')[0],c[j],T0,gamma,loop,Mw,P0,ood[od_num])
...
plt.plot(q_l,d0_l,label = 'd0 vs gamma')
plt.plot(q_l,L_l,label = 'L vs gamma')
plt.plot(q_l,W_l,label = 'W vs gamma')
plt.grid()
plt.title('q',size=14)
...
plt.savefig('q_change.png')
plt.clf()
#plt.show()

plt.plot(q_l,theta_l,label = 'theta')
plt.grid()
plt.ylim([0,50])
plt.title('q',size=14)
...
j = 1
'''
gamma_l = np.linspace(1.23,1.35,50,endpoint=True)
print('gamma loop')
d0_l, L_l, W_l, theta_l = [],[],[],[]
#gamma loop
print(dt.datetime.now())
...
plt.legend(loc='upper left',fontsize = 14)
plt.savefig('gamma_change_p.png')
plt.clf()
#plt.show()
...
#mol loop
mol_l = np.linspace(0.032,0.04,30,endpoint=True)
print('Mw loop')
d0_l, L_l, W_l, theta_l = [],[],[],[]
print(dt.datetime.now())
for loop in mol_l:
