...
for od in od_l:
    x = blast(lu_et.get('eff4')[0],4,T0,gamma,q_hat,Mw,P0,od)
    j+=1
...
L_l2 = np.array(L_l)/100
ans_l = np.full_like(od_l,7.2/100)
ans2_l = np.full_like(od_l,7.2)
plt.figure(figsize=(7, 5))
plt.plot(od_l,L_l2,label = 'model results')
plt.plot(od_l,ans_l,label= 'simulation results')
plt.grid()
plt.xticks(size=14)
plt.yticks(size=14)
plt.ylim(0.05,0.1)
