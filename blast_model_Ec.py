...
def find_t_star(t_star):
    ...
    return ((t_star+tau)-opt.fsolve(t_f,1.5*t_star))
...
def geo_mod(d0): #calculate r at t[0,t*]
    ...
    r0        = d0/2
    ############################################
    t_star    = opt.root(find_t_star,15*r0/D0).x#t_star    = ((np.sqrt(2)-1))*(od*r0/D0)opt.bisect(find_t_star,0.1*r0/D0,10*r0/D0)#opt.minimize(find_t_star,30*r0/D0,method='nelder-mead').x#
    r_star    = r(t_star)#(r0)*np.sqrt((1+od**2))
    t_end      = opt.fsolve(t_final,1.3*t_star)
    if t_end<1e-12:
        return 100
...
def astric(tt,rr):
    return np.abs(rr-r(tt))

def blast(AA,eff,T00,gam,qq,moll,P00,odd):
    ############################################
        #units SI
    global D0, C0, gamma,rho0,T0,P0,RR,Rr,alpha,h,A,Ea,q,rho_vn0, T_vn0, P_vn0, r_l, M_l, L, P3, od, r0
    vec       = [1500,2500,2000]
    ...
    d0 =  opt.root_scalar(geo_mod,method='secant',x0=0.004,x1=0.002)
    ...
    r_ast = r0*np.sqrt(od**2+1)
    t_ast = opt.root(astric,t_star/2,args=(r_ast)).x
    E0= alpha*rho0*(D_od*d0.root)**2
    E1_est= 2*rho0*(D0*od*d0.root)**2*alpha
    Ecc=4*alpha*rho0*r_ast**2*D(t_ast)**2
    print(t_ast,r_ast,Ecc,E1_est)
    print(od,'od =',np.sqrt(2)*((D(t_ast)*r_ast)/(D0*d0.root)))
    print(D_l[-1])
    '''
    print('3 compression')
