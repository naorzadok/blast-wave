...
def find_t_star(t_star):
    ...
    return ((t_star+tau)-opt.fsolve(t_f,1.3*t_star))
...
def geo_mod(d0): #calculate r at t[0,t*]
    ...
    r0        = d0/2
    ############################################
    t_star    = opt.root(find_t_star,20*r0/D0).x#t_star    = ((np.sqrt(2)-1))*(od*r0/D0)opt.bisect(find_t_star,0.1*r0/D0,10*r0/D0)#opt.minimize(find_t_star,30*r0/D0,method='nelder-mead').x#
    r_star    = r(t_star)#(r0)*np.sqrt((1+od**2))
    t_end      = opt.fsolve(t_final,1.3*t_star)
    print('1to', t_star,t_end,t_end/t_star)
    if t_end<1e-12:
        return 100
    time_jump  = t_star/1000
    t_arr      = np.arange(0, t_end+time_jump,time_jump) # t_arr      = np.arange(0,t_end+time_jump,time_jump)
...
def blast(AA,eff,T00,gam,qq,moll,P00,odd):
    ...
    d0 =  opt.root_scalar(geo_mod,method='secant',x0=0.006,x1=0.003)#(geo_mod,method='secant',x0=0.005,x1=0.003)
    #d0  =opt.bisect(geo_mod,0.0005,0.5)#,xtol=0.00001)
    #d0 = 0.005#0.005
    #r0        = d0.x/2
    r0        = d0.root/2
    ############################################
    t_star    = opt.root(find_t_star,15*r0/D0).x#opt.fsolve(find_t_star,15*r0/D0)#t_star    = ((np.sqrt(2)-1))*(od*r0/D0)opt.bisect(find_t_star,0.1*r0/D0,10*r0/D0)#
    r_star    = r(t_star)#(r0)*np.sqrt((1+od**2))
    t_end      = opt.fsolve(t_final,1.3*t_star)
    
    time_jump  = t_star/1000
    print('2to', t_star,t_end,t_end/t_star,time_jump)
    t_arr      = np.arange(0, t_end+time_jump,time_jump) # t_arr      = np.arange(0,t_end+time_jump,time_jump)
    #############################################
    r_l       = np.fromiter(map(r, t_arr), dtype=np.float)
    D_l       = np.fromiter(map(D, t_arr), dtype=np.float)
