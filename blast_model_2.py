...
def find_t_star(t_star):
    ...
    return ((t_star+tau)-opt.fsolve(t_f,2*t_star))
...
def startup(x,gamm,mo,qu,TT):
    ...
    #print(16*x[1]/x[0]) #effective activation energy
    #print(x)
    return [eq1,eq2,eq3]
...
def reflection(theta):#, P3,M_arr,d0,od):
    ...
    rho3   = (((gamma+1)*M_R.x**2)/(2+(gamma-1)*M_R.x**2))*rho2
    T3     = (PP3/(rho3*RR))
    print('T ref' , T3)
...
def find_od(ood):
    #if ood<1.0:
    #    return math.inf
    P4        = (alpha*rho0*(D0*ood)**2*(gamma-1))/(h)
    T4        = (P4/P0)**((gamma-1)/gamma)*T0
    a_rat     = np.sqrt(T0/T4)
    M0        = (D0*ood)/C0
    P2        = ((1+(2*gamma)/(gamma+1)*(M0**2-1)))*P0
    eq        = P4-P2
    #eq = (P4/P0)-(P2/P0)*(1-(((gamma-1)*a_rat*(P2/P0-1))/(np.sqrt(2*gamma*(2*gamma+(gamma+1)*(P2/P0-1))))))**(-(2*gamma)/(gamma-1))
    print('od',ood,P0,T0,P2,P4,T4,M0,eq)
    print(alpha,rho0,M0)
    #print((((gamma-1)*a_rat*(P2/P0-1))/(np.sqrt(2*gamma*(2*gamma+(gamma+1)*(P2/P0-1))))))
    return eq
...
def find_P4(P4):
    ...
    P2        = ((1+(2*gamma)/(gamma+1)*(M0**2-1)))*P0
    eq = (P4/P0)-(P2/P0)*(1-(((gamma-1)*a_rat*(P2/P0-1))/(np.sqrt(2*gamma*(2*gamma+(gamma+1)*(P2/P0-1))))))**(-(2*gamma)/(gamma-1))
    print(P4,P2,a_rat,T4,eq)
    return eq

def blast(AA,eff,T00,gam,qq,moll,P00):
    ############################################
    
        #units SI
    global D0, C0, gamma,rho0,T0,P0,RR,Rr,alpha,h,A,Ea,q,rho_vn0, T_vn0, P_vn0, r_l, M_l, L, P3, od, r0, h, alpha
    vec       = [1500,2500,2000]
    ...
    P4         = opt.root_scalar(find_P4,method='secant',x0=100*P0,x1=200*P0)
    ody        = opt.root_scalar(find_od,method='secant',x0=1.6,x1=1.7)
    print(ody)
    od = 1.7#od.root
    ...
    d0 =  opt.root_scalar(geo_mod,method='secant',x0=0.007,x1=0.004)
    #d0  =opt.bisect(geo_mod,0.0005,0.5)#,xtol=0.00001)
    #d0 = 0.005#0.005
    #r0        = d0.x/2
    r0        = d0.root/2
    ############################################
    t_star    = opt.fsolve(find_t_star,30*r0/D0)#t_star    = ((np.sqrt(2)-1))*(od*r0/D0)opt.bisect(find_t_star,0.1*r0/D0,10*r0/D0)#
    r_star    = r(t_star)#(r0)*np.sqrt((1+od**2))
    t_end      = opt.fsolve(t_final,2*t_star)
